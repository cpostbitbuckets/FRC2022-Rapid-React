...
public class SwerveModuleSim {
  ...
  void update(double dtSeconds) {
    ...
    double velocityAlongAzimuth = getModuleRelativeTranslationVelocity(
      dtSeconds
    )
      .dot(azimuthUnitVec);
    ...
    curAzmthAngle =
      Rotation2d.fromDegrees(azmthMotor.getMechanismPosition_Rev() * 360);
  }
  ...
  Vector2d getModuleRelativeTranslationVelocity(double dtSeconds) {
    double xVel =
      (
        curModulePose.getTranslation().getX() -
        prevModulePose.getTranslation().getX()
      ) /
      dtSeconds;
    double yVel =
      (
        curModulePose.getTranslation().getY() -
        prevModulePose.getTranslation().getY()
      ) /
      dtSeconds;
    Vector2d moduleTranslationVec = new Vector2d(xVel, yVel);
    moduleTranslationVec.rotate(
      -1.0 * curModulePose.getRotation().getDegrees()
    );
   ...
   * @return
   */
  ForceAtPose2d getCrossTreadFrictionalForce(
    Force2d netForce_in,
    double dtSeconds
  ) {
    ...
    crossTreadVelMag =
      getModuleRelativeTranslationVelocity(dtSeconds).dot(crossTreadUnitVector);
    crossTreadForceMag = netForce_in.getVector2d().dot(crossTreadUnitVector);

    Force2d fricForce = new Force2d();

    if (
      Math.abs(crossTreadForceMag) > treadStaticFricForce ||
      ...
      crossTreadFricForceMag =
        -1.0 * Math.signum(crossTreadVelMag) * treadKineticFricForce;
    } else {
  ...
  ForceAtPose2d getWheelMotiveForce() {
    return new ForceAtPose2d(
      new Force2d(wheelMotor.getGroundForce_N(), curAzmthAngle),
      curModulePose
    );
  }
