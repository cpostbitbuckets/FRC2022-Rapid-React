package frc.robot.simulator.swerve;
...
public class QuadSwerveSim {
  ...
  public void update(double dtSeconds) {
    ...
    Transform2d fieldToRobotTrans = new Transform2d(
      fieldReferenceFrame,
      curPose
    );
    ...
    ForceAtPose2d preFricNetForceRobotCenter = new ForceAtPose2d(
      preFricNetForce,
      curPose
    );

    // Calculate the forces from cross-tread friction at each module
    ArrayList<ForceAtPose2d> netXtreadFricForces = new ArrayList<ForceAtPose2d>(
      NUM_MODULES
    );
    for (int idx = 0; idx < NUM_MODULES; idx++) {
      SwerveModuleSim mod = modules.get(idx);
      double perWheelForceFrac = 1.0 / NUM_MODULES; //Assume force evenly applied to all modules.
      Force2d preFricForceAtModule = preFricNetForceRobotCenter
        .getForceInRefFrame(mod.getModulePose())
        .times(perWheelForceFrac);
      netXtreadFricForces.add(
        mod.getCrossTreadFrictionalForce(preFricForceAtModule, dtSeconds)
      );
    }
    ...
    Force2d robotForceInFieldRefFrame = netForce.getForceInRefFrame(
      fieldReferenceFrame
    );
    ...
    Vector2d accel = robotForceInFieldRefFrame
      .times(1 / robotMass_kg)
      .getVector2d();
    ...
    Translation2d posChange = new Translation2d(
    ...
    Twist2d motionThisLoop = new Twist2d(
      posChange.getX(),
      posChange.getY(),
      rotPosChange
    );
